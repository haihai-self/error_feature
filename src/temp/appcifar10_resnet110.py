...
def resnet_layer(inputs,
    ...
    if kernel_size != 1:
        conv = FakeApproxConv2D(num_filters,
                      kernel_size,
                      strides=strides,
                      padding='same',
                      kernel_initializer='he_normal',
                      kernel_regularizer=l2(1e-4),
                      mul_map_file=mul_name
                      )
    else:
...
def resnet_v1(input_shape, depth, src="", num_classes=10):
    """ResNet 版本 1 模型构建器 [a]
...
def resnet_v2(input_shape, depth, src="", num_classes=10):
    """ResNet 版本 2 模型构建器 [b]
...
def appTest(name, file, result_path, mul_name, src=""):
    ...
    """
    if version == 2:
        model = resnet_v2(input_shape=input_shape, depth=depth, src=src)
    else:
        model = resnet_v1(input_shape=input_shape, depth=depth, src=src)
