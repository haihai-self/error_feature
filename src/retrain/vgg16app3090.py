...
def vgg16app(app_src, shape):
    img_input = tf.keras.Input(shape=shape, name='img_input')
    x = FakeApproxConv2D(
        64, (3, 3), activation='relu', padding='same', name='block1_conv1', mul_map_file=app_src)(img_input)
...
def trainApp(mul_dirs, name, log_dir):
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "cifar10":
        ...
        print("dataset mnist")
        x_train, y_train, x_test, y_test, image_gen = loaddata.mnist()
        shape=(32, 32, 1)


    model = vgg16app(app_src, shape)
    # model.summary()
    ...
    cp_callback = tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_path + "/weights", save_weights_only=True, monitor='val_accuracy',
                                                     save_best_only=True)
    print(name)
    if len(sys.argv) > 2 and sys.argv[2] == "image_gen":
...
if __name__ == '__main__':
    ...
    with open(path +"/src/retrain/mul_name.log", "r") as f:
        for line in f.readlines():
            need_train.add(line.split()[0])
    need_train = list(need_train)
    need_train = sorted(need_train)
    if len(sys.argv) > 1 and sys.argv[1] == "cifar10":
        print("dataset cifar10")
        temp = 'cifar10'
    ...
    if len(sys.argv) > 2 and sys.argv[2] == "image_gen":
        ...
        temp += '_origin'
    mul_dirs = sorted(mul_dirs)
    for i in range(len(mul_dirs)):
        log_dir = path + '/result/retrain/vgg16_' + temp + '_app/' + mul_dirs[i]
        if not os.path.exists(log_dir):
            os.makedirs(log_dir)
        mul_lists = os.listdir(mul_path + mul_dirs[i] + "/Bin")
        mul_lists = sorted(mul_lists)
        trained_list_temp = os.listdir(log_dir)
        trained_list = []
        for trained in trained_list_temp:
            if os.path.isfile(log_dir + '/' + trained + "/acc.log"):
                trained_list.append(trained)
            else:
                shutil.rmtree(log_dir + '/' + trained)
